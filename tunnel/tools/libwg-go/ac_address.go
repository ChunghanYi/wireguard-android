...
func GetMacAddress(mac []byte) bool {
	...
	if err != nil {
		acLogDebug("[AC] Error getting network interfaces: %v", err)
		return false
	}

	for _, iface := range interfaces {
		m := iface.HardwareAddr
		if len(m) > 0 { // Check if a MAC address exists for the interface
			acLogDebug("[AC] Interface: %s, MAC Address: %s", iface.Name, m.String())
			copy(mac, iface.HardwareAddr)
			return true
		} else {
			acLogDebug("[AC] Interface: %s, No MAC Address found\n", iface.Name)
		}
	}
